export class JournalLink {
    ...
    async initNewPage(entity, change) {    
        //Make shure references are not carried over to copies
    ...
    async updateActor(entity, change) {
        ...
        if(content===undefined) {                        
            content=change.system?.notes?.source;        
        }
    ...
    async update(entity, entityType, content, force) {
        ...
        for (let reference of references) {
            ...
            try {
                ...
                let msg="Link-Tarket in ";
                if(entity.parent!==undefined) {
                    msg+=entity.parent.name+"->";
                }
                msg+=entity.name +" does not exist";
                this.log(msg);
            }
    ...
    includeLinks(html, entityData) {
        ...
        $("body > .journal-backlinks").remove();
        $('form.editable  + div.journal-backlinks').remove();
        //$(".journal-entry-page > .journal-backlinks").remove();
    }
    ...
    async sync() {
        ...
        let entries = game.documentIndex.lookup('', {documentTypes: document_types,limit:100000000});
